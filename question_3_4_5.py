...
# ЭТА ЧАСТЬ ИСПОЛНЯЕТСЯ ЕСЛИ ЗАПУСК БЫЛ С АРГУМЕНТАМИ КОМАНДНОЙ СТРОКИ
if len(argv) == 2:      # если имеется 1 аргумент запуска
    ...
    if argv[1] == 'gen':                                # и если первый - запрос на генерацию хобби
        ...
        print('Exit code 2: Указаны неверные аргументы')                 # сообщаем об ошибке
    ...
    if exists(argv[1]) and exists(argv[2]):             # и файлы с именами соответствующими 1 и 2 аргументу существуют
        ...
        print('Exit code 3: Вы указали не существующий файл')            # выводим сообщение об ошибке

# ЭТА ЧАСТЬ ИСПОЛНЯЕТСЯ ПРИ ЗАПУСКЕ НАПРЯМУЮ ИЗ PYCHARM ИЛИ БЕЗ АРГУМЕНТОВ
